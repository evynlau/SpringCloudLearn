...
//        int age = 10/0;
    ...
    @HystrixCommand(fallbackMethod = "circuitBreakerHandler", commandProperties = {
            ...
            @HystrixProperty(name = "circuitBreaker.sleepWindowInMilliseconds", value = "10000"),//快照时间窗口，默认10秒
            @HystrixProperty(name = "circuitBreaker.errorThresholdPercentage", value = "60"),//错误百分比阈值
            //在快照时间窗口，最近10秒钟内，请求总数10次中错误60%，将开启断路器。此时所有请求不会进行转发
            //一段时间后（默认5秒），断路器处于半开状态，尝试转发其中的请求，若成功，断路器关闭。
            //若失败继续上一步。
             ...
             * Command Properties
             * Execution相关的属性的配置：
             * hystrix.command.default.execution.isolation.strategy 隔离策略，默认是Thread, 可选Thread｜Semaphore {@link HystrixCommandProperties.ExecutionIsolationStrategy}
    ...
    public String circuitBreaker(Integer id) {
        ...
        if (id < 0) {
            throw new RuntimeException("*******id 不能为负 抛出异常");
        }
        String serialNum = IdUtil.simpleUUID();

        return Thread.currentThread().getName() + "  调用成功，serialNumber is " + serialNum;
    }

    public String circuitBreakerHandler(Integer id) {
        return "id:" + id + "错误，请稍后再试~~/(ㄒoㄒ)/~~";
    }
}
